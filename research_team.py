...
class ResearchTeam:
    ...
    async def run(self, query, iterations=3):
        tasks_researchers = [
            researcher.run(query) for researcher in self.research_actor_instance
        ]
        result_researchers = await asyncio.gather(*tasks_researchers)
        list_reports = [
            {"report": result[0], "paragraphs": result[1]}
            for result in result_researchers
        ]
        ...
        if self.use_reflection:
            for i in range(iterations):
                ...
                if len(list_reports) > 1:
                    ...
                    list_reports, immedia_report = await self.ranker_actor_instance.run(
                        list_reports, query
                    )
                ...
                if response["accept"]:
                    ...
                    revised_report = await self.revise_actor_instance.run(
                        markdown_report, response["notes"]
                    )
