...
from erniebot_agent.tools.langchain_retrieval_tool import \
...
class GenerateAbstract:
    def __init__(
        self, llm, chunk_size: int = 1500, chunk_overlap=0, path="./abstract.json"
    ):
    ...
    async def run(self, data_dir, url_path=None):
        ...
        for item in docs:
            ...
            summary_list.append(
                {"page_content": summary, "url": url, "name": item.metadata["source"]}
            )
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Full-text data storage folder path",
    )
    parser.add_argument(
        "--path_abstract", type=str, default="", help="json file path to store summary"
    )
    parser.add_argument(
        "--embedding_type",
        type=str,
    ...
    parser.add_argument(
        "--url_path", type=str, default="", help="txt file path to store url link"
    )
    parser.add_argument(
        "--framework",
        type=str,
...
if __name__ == "__main__":
    ...
    if not args.path_abstract:
        ...
        abstract_path = asyncio.run(
            generate_abstract.run(args.path_full_text, args.url_path)
        )
    else:
        abstract_path = args.path_abstract

    if args.framework == "llmama_index":
