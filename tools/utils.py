...
class ReportCallbackHandler(LoggingHandler):
    ...
    async def on_tool_end(
        self, agent: Any, tool: Union[BaseTool, str], response: Any
    ) -> None:
...
def build_index(
    ...
    if os.path.exists(faiss_name):
        db = FAISS.load_local(
            faiss_name, embeddings, allow_dangerous_deserialization=True
        )
        ...
        with jsonlines.open(path) as reader:
            for obj in reader:
                if type(obj) is list:
                    ...
                    documents = Document(
                        page_content=obj["page_content"], metadata=metadata
                    )
                    docs = text_splitter.split_documents([documents])
                    all_docs.extend(docs)
        ...
        text_splitter = SpacyTextSplitter(
            pipeline="zh_core_web_sm", chunk_size=chunk_size, chunk_overlap=0
        )
        docs = text_splitter.split_documents(documents)
        docs_tackle = []
        for item in docs:
...
def convert_markdown_to_pdf(markdown_content: str, output_pdf_file: str):
    ...
    HTML(string=html_content).write_pdf(
        output_pdf_file, stylesheets=[css], font_config=font_config
    )
...
def add_citation(paragraphs, index_name, embeddings, build_index, searchTool):
    ...
    faiss_db = build_index(
        index_name=index_name, embeddings=embeddings, origin_data=list_data
    )
