...
class FactCheckerAgent(JsonUtil):
    ...
    def __init__(
        ...
        self.prompt_anwser = PromptTemplate(
            ANWSER_PROMPT, input_variables=["question", "content"]
        )
    ...
    async def generate_anwser(self, question: str, context: str):
        ...
        messages: List[Message] = [
            HumanMessage(
                content=self.prompt_anwser.format(question=question, content=context)
            )
        ]
    ...
    async def check_claim(self, question: str, answer: str, claim: str):
        ...
        messages: List[Message] = [
            HumanMessage(
                content=self.prompt_check_claim.format(
    ...
    async def verifications(self, facts_problems: List[dict]):
        ...
        for item in facts_problems:
            ...
            if result["is_correct"] is False:
                ...
                item["modify"] = claim
        self._callback_manager._agent_info(
            msg=item["modify"], subject="事实验证的结果", state="End"
        )
    ...
    async def generate_final_response(self, content: str, verifications: List[dict]):
        ...
        if all([item["is_correct"] for item in verifications]):
            ...
            messages: List[Message] = [
                HumanMessage(
                    content=self.prompt_final_response.format(
                        origin_content=content, context=context
                    )
                )
            ]
        ...
        for item in report_list:
            if item.strip()[0] == "#":
                ...
                if contains_numbers:
                    messages: List[Message] = [
                        HumanMessage(
                            content=self.prompt_plan_verifications.format(
                                base_context=item
                            )
                        )
                    ]
                    ...
                    new_item: str = await self.generate_final_response(
