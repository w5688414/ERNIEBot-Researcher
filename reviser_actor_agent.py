...
class ReviserActorAgent(JsonUtil):
    ...
    def __init__(
        ...
        self.prompt_template = PromptTemplate(
            template=self.template, input_variables=["draft", "notes"]
        )
    ...
    async def run(self, draft: Union[str, Dict], notes: str) -> str:
        if isinstance(draft, dict):
            await self._callback_manager.on_run_start(
                agent=self, prompt=draft["report"]
            )
        else:
    ...
    async def _run(self, draft, notes):
        ...
        if isinstance(draft, dict):
            content = self.prompt_template.format(
                draft=draft["report"], notes=notes
            ).replace(". ", ".")
        else:
            ...
            try:
                if len(content) > TOKEN_MAX_LENGTH:
                    response = await self.llm_long.chat(
                        messages=messages, system=self.system_message
                    )
                else:
